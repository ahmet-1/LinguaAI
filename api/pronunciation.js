...
module.exports = async (req, res) => {
  ...
  try {
    ...
    else if (langCode.includes('en')) langCode = 'en-US';
    else if (langCode.includes('tr')) langCode = 'tr-TR';
    ...
    const azureReq = https.request(options, (azureRes) => {
      ...
      azureRes.on('end', () => {
        try {
          ...
          return res.status(200).json({
            error: 'Ses algılanamadı. Lütfen mikrofona biraz daha yakın ve net okuyun.'
          });
        } catch (e) {
